    ...
    render() {
        return  <div className="dashboard-horizontal-box-item" style={ this.state.dropdownToggle ? {minHeight: 'auto'} : {} } >
                    <div className={`dashboard-horizontal-box-item-icon  flex center icon-bg-${this.props.iconColor}`}>
                        { this.props.activity === "running" ? <FA.FaRunning color={'#fff'} fontSize={'1.35rem'} /> : <></> }
                        { this.props.activity === "swimming" ? <FA.FaSwimmer color={'#fff'} fontSize={'1.35rem'} /> : <></> }
                        { this.props.activity === "biking" ? <FA.FaBiking color={'#fff'} fontSize={'1.35rem'} /> : <></> }
                    </div>
